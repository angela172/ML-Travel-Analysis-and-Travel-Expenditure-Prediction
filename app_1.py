...
#---------------------------------SIDE BAR OPTIONS---------------------------------------------
    ...
    options2 = ["Random Forest", "Decision Tree" ]
    with st.form(key='trav_form'):
        selected_option2 = st.selectbox("Select a Model: ", options2)
        submit_button = st.form_submit_button(label='Submit')
        if submit_button:
            markdown_text = f" Option selected: `{selected_option2}\n`"
            st.sidebar.markdown(markdown_text)
            ...
            if selected_option2 == "Decision Tree":
                st.write("Here's a plot that showcases the predicted Y values and the actual Y values for Decision Tree Regression")
                st.write("R^2: 1.")
                im3 = Image.open(r"C:\Users\11 PrO\Downloads\new ml\down2.png")
                st.image(im3, width = 500)
    ...
    Accommadation cost and transportation cost when trip planning. The dataset for this project has been taken from Kaggle.
    
    The required features that have been selected from the original dataset for these models are: 
    ...
    markdown3= '''
    Moving on, The technique used for the purpose of this project is regression. Out of the many regression models,Decision Tree Regressor, Random Forest Regressor and Logistic Regression tend to give
    a high R-square value for predicting the Accommodation cost and Decision tree regressor tends to give a good R-square value for Transportation cost.
    '''
